...
const TransactionForm = (TransactionFormProps: TransactionFormProps) => {
    ...
    return (
        <form className={`bg-gray p-6 rounded-xl border border-light-yellow border-opacity-50 lg:sticky top-40 max-h-[65svh] flex flex-col justify-between ${showForm ? "animate-openForm" : (showForm !== undefined ? "animate-closeForm" : "lg:w-[30%] md:w-[60%] w-full")}`}>
            <FontAwesomeIcon className='absolute top-1/2 -translate-y-1/2 translate-x-1/2 right-0 text-black bg-login-button px-2 py-1 rounded-md max-lg:hidden' onClick={() => setShowForm(false)} icon={faAnglesLeft} />
                ...
                <div className='flex gap-4 mt-2'>
                    ...
                    <div className="relative flex-[0.35]">
                        <Select<CurrencyType>
                            ...
                            dataIndex='id'
                            displayKey='code'
                            onChange={(id) => updateTransactionForm({ currency_id: Number(id) })}
                            style={{ height: "45px" }}
                            search
